...
- **Validated** — narrow claim supported by tests and/or focused comparison on the working line
...
| Element volume-velocity export through `os_lem.api` | Released | promoted facade path shipped on the released `v0.3.0` line | v0.3.0 | supported target types remain narrow |
| Element particle-velocity export through `os_lem.api` | Released | promoted facade path shipped on the released `v0.3.0` line | v0.3.0 | supported target types remain narrow |
...
| Bounded lossy conical `waveguide_1d` | Validated | implemented parser/solver path plus focused validation, maintained example, and refinement coverage on the `v0.4.0` working line | v0.4.0 | bounded working-line claim only |
| Maintained conical-line hero example | Validated | refreshed example plus regression smoke on the `v0.4.0` working line | v0.4.0 | one maintained example only |
| Segmentation-refinement validation for the official conical example | Validated | frozen 32-vs-64 impedance/profile metrics on the `v0.4.0` working line | v0.4.0 | official reference case only |
...
| Distributed losses beyond current bounded waveguide-loss support | Exploratory | not in current delivered subset | later | do not claim yet |
