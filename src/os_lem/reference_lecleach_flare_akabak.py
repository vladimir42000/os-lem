...
from .reference_lecleach_flare_smoke import lecleach_model_dict
from .solve import radiator_observation_pressure, solve_frequency_sweep

LECLEACH_FLARE_AKABAK_TOPOLOGY = (
    "one rear volume behind the driver, one Le Cleac'h-only rear-to-mouth flare path, "
    "one direct front radiator, and one mouth radiator"
)

LECLEACH_FLARE_AKABAK_REFERENCE_KIND = "akabak_oriented_segmented_conical_surrogate"

LECLEACH_FLARE_AKABAK_LIMITATIONS = (
    "This bundle is an Akabak-oriented smoke surface for the Le Cleac'h-only flare path: the reference files use "
    ...
    "The bundled reference script approximates the same Le Cleac'h flare with a fixed 12-section conical chain because "
...
@dataclass(slots=True, frozen=True)
class LeCleachFlareAkabakReferenceBundle:
...
@dataclass(slots=True, frozen=True)
class LeCleachFlareAkabakComparison:
    frequency_hz: np.ndarray
...
def load_lecleach_flare_akabak_reference_bundle(
    reference_dir: str | Path,
) -> LeCleachFlareAkabakReferenceBundle:
    ref_dir = Path(reference_dir)
    impedance = _load_two_column_series(
        ref_dir / "LEC_Z.ZMA",
        unit="ohm",
        label="Akabak-oriented Le Cleac'h-flare input impedance magnitude",
    )
    pressure_db = _load_two_column_series(
        ref_dir / "LEC_LP.FRD",
        unit="dB",
        label="Akabak-oriented Le Cleac'h-flare mouth pressure level",
    )
    pressure_pa = _load_two_column_series(
        ref_dir / "LEC_AMP.FRD",
        unit="Pa",
        label="Akabak-oriented Le Cleac'h-flare mouth pressure amplitude",
    )
    pressure_phase = _load_two_column_series(
        ref_dir / "LEC_PHS.FRD",
        unit="deg",
        label="Akabak-oriented Le Cleac'h-flare mouth pressure phase",
    )
    ...
    if not (
        ...
        raise ValueError("Le Cleac'h flare reference files do not share an identical frequency grid")

    script_text = (ref_dir / "LEC.aks").read_text(encoding="latin1")
    return LeCleachFlareAkabakReferenceBundle(
...
def build_lecleach_flare_model_dict() -> dict[str, Any]:
    model_dict = copy.deepcopy(lecleach_model_dict())
    model_dict["meta"] = {
        "name": "lecleach_flare_akabak_oriented_smoke",
        "radiation_space": "2pi",
    }
...
def build_lecleach_flare_segmented_conical_reference_model_dict() -> dict[str, Any]:
    model_dict = build_lecleach_flare_model_dict()
    ...
    for idx in range(segments):
        conical_segments.append(
            {
                "id": f"lec_seg_{idx + 1:02d}",
                "type": "waveguide_1d",
                "node_a": "rear" if idx == 0 else f"lec_node_{idx}",
                "node_b": "mouth" if idx == segments - 1 else f"lec_node_{idx + 1}",
    ...
    model_dict["meta"]["name"] = "lecleach_flare_akabak_segmented_conical_reference"
    return model_dict
...
def compare_lecleach_flare_against_akabak_oriented_reference(
    reference_dir: str | Path,
) -> LeCleachFlareAkabakComparison:
    bundle = load_lecleach_flare_akabak_reference_bundle(reference_dir)
    model, warnings = normalize_model(build_lecleach_flare_model_dict())
    if warnings:
        raise ValueError(f"Le Cleac'h flare smoke model produced unexpected normalization warnings: {warnings}")
    ...
    metrics: dict[str, float | bool | str] = {
        ...
        "reference_kind": LECLEACH_FLARE_AKABAK_REFERENCE_KIND,
        ...
        "case_topology": LECLEACH_FLARE_AKABAK_TOPOLOGY,
    }

    return LeCleachFlareAkabakComparison(
        ...
        metrics=metrics,
        limitations=LECLEACH_FLARE_AKABAK_LIMITATIONS,
        topology=LECLEACH_FLARE_AKABAK_TOPOLOGY,
    )


def write_lecleach_flare_akabak_compare_outputs(
    reference_dir: str | Path,
    outdir: str | Path,
) -> LeCleachFlareAkabakComparison:
    comparison = compare_lecleach_flare_against_akabak_oriented_reference(reference_dir)
    out_path = Path(outdir)
    out_path.mkdir(parents=True, exist_ok=True)

    csv_path = out_path / "lecleach_flare_akabak_compare.csv"
    header = (
    ...
    summary = {
        "case": "Le Cleac'h flare Akabak-oriented smoke",
