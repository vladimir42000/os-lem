...
class TractrixSmokeObservation:
    segments: int
    x_m: np.ndarray
...
@dataclass(frozen=True)
class TractrixSmokeReport:
    observations: tuple[TractrixSmokeObservation, ...]
    frequency_hz: float


def tractrix_model_dict() -> dict:
    return {
        "driver": {
            "id": "drv1",
            "model": "ts_classic",
...
def build_tractrix_model_with_segments(segments: int) -> dict:
    ...
    for element in model_dict["elements"]:
        if element.get("id") == "wg1":
            ...
            return model_dict
    raise AssertionError("expected wg1 element in tractrix smoke model")
...
def evaluate_tractrix_reference_smoke(
    ...
    *,
    ...
    observations: list[TractrixSmokeObservation] = []
    ...
    return TractrixSmokeReport(observations=tuple(observations), frequency_hz=float(frequency_hz))
