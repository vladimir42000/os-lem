...
class LeCleachSmokeObservation:
    segments: int
    x_m: np.ndarray
...
@dataclass(frozen=True)
class LeCleachSmokeReport:
    observations: tuple[LeCleachSmokeObservation, ...]
    frequency_hz: float


def lecleach_model_dict() -> dict:
    return {
        "driver": {
            "id": "drv1",
            "model": "ts_classic",
...
def build_lecleach_model_with_segments(segments: int) -> dict:
    ...
    for element in model_dict["elements"]:
        if element.get("id") == "wg1":
            ...
            return model_dict
    raise AssertionError("expected wg1 element in Le Cleac'h smoke model")
...
def evaluate_lecleach_reference_smoke(
    ...
    *,
    ...
    observations: list[LeCleachSmokeObservation] = []
    ...
    return LeCleachSmokeReport(observations=tuple(observations), frequency_hz=float(frequency_hz))
