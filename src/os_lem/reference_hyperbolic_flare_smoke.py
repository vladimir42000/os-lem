...
class HyperbolicSmokeObservation:
    segments: int
    x_m: np.ndarray
...
@dataclass(frozen=True)
class HyperbolicSmokeReport:
    observations: tuple[HyperbolicSmokeObservation, ...]
    frequency_hz: float


def hyperbolic_model_dict() -> dict:
    return {
        "driver": {
            "id": "drv1",
            "model": "ts_classic",
...
def build_hyperbolic_model_with_segments(segments: int) -> dict:
    ...
    for element in model_dict["elements"]:
        if element.get("id") == "wg1":
            ...
            return model_dict
    raise AssertionError("expected wg1 element in hyperbolic smoke model")
...
def evaluate_hyperbolic_reference_smoke(
    ...
    *,
    ...
    node_a: str = "rear",
    node_b: str = "mouth",
) -> HyperbolicSmokeReport:
    observations: list[HyperbolicSmokeObservation] = []
    ...
    return HyperbolicSmokeReport(observations=tuple(observations), frequency_hz=float(frequency_hz))
