...
from .reference_parabolic_flare_smoke import parabolic_model_dict
from .solve import radiator_observation_pressure, solve_frequency_sweep

PARABOLIC_FLARE_AKABAK_TOPOLOGY = (
    "one rear volume behind the driver, one parabolic-only rear-to-mouth flare path, "
    "one direct front radiator, and one mouth radiator"
)

PARABOLIC_FLARE_AKABAK_REFERENCE_KIND = "akabak_oriented_segmented_conical_surrogate"

PARABOLIC_FLARE_AKABAK_LIMITATIONS = (
    "This bundle is an Akabak-oriented smoke surface for the parabolic-only flare path: the reference files use "
    ...
    "The bundled reference script approximates the same parabolic flare with a fixed 12-section conical chain because "
...
@dataclass(slots=True, frozen=True)
class ParabolicFlareAkabakReferenceBundle:
...
@dataclass(slots=True, frozen=True)
class ParabolicFlareAkabakComparison:
    frequency_hz: np.ndarray
...
def load_parabolic_flare_akabak_reference_bundle(
    reference_dir: str | Path,
) -> ParabolicFlareAkabakReferenceBundle:
    ref_dir = Path(reference_dir)
    impedance = _load_two_column_series(
        ref_dir / "PAR_Z.ZMA",
        unit="ohm",
        label="Akabak-oriented parabolic-flare input impedance magnitude",
    )
    pressure_db = _load_two_column_series(
        ref_dir / "PAR_LP.FRD",
        unit="dB",
        label="Akabak-oriented parabolic-flare mouth pressure level",
    )
    pressure_pa = _load_two_column_series(
        ref_dir / "PAR_AMP.FRD",
        unit="Pa",
        label="Akabak-oriented parabolic-flare mouth pressure amplitude",
    )
    pressure_phase = _load_two_column_series(
        ref_dir / "PAR_PHS.FRD",
        unit="deg",
        label="Akabak-oriented parabolic-flare mouth pressure phase",
    )
    ...
    if not (
        ...
        raise ValueError("Parabolic flare reference files do not share an identical frequency grid")

    script_text = (ref_dir / "PAR.aks").read_text(encoding="latin1")
    return ParabolicFlareAkabakReferenceBundle(
...
def build_parabolic_flare_model_dict() -> dict[str, Any]:
    model_dict = copy.deepcopy(parabolic_model_dict())
    model_dict["meta"] = {
        "name": "parabolic_flare_akabak_oriented_smoke",
        "radiation_space": "2pi",
    }
...
def build_parabolic_flare_segmented_conical_reference_model_dict() -> dict[str, Any]:
    ...
    for idx in range(segments):
        conical_segments.append(
            {
                "id": f"par_seg_{idx + 1:02d}",
                "type": "waveguide_1d",
                "node_a": "rear" if idx == 0 else f"par_node_{idx}",
                "node_b": "mouth" if idx == segments - 1 else f"par_node_{idx + 1}",
    ...
    model_dict["meta"]["name"] = "parabolic_flare_akabak_segmented_conical_reference"
    return model_dict
...
def compare_parabolic_flare_against_akabak_oriented_reference(
    reference_dir: str | Path,
) -> ParabolicFlareAkabakComparison:
    bundle = load_parabolic_flare_akabak_reference_bundle(reference_dir)
    model, warnings = normalize_model(build_parabolic_flare_model_dict())
    if warnings:
        raise ValueError(f"Parabolic flare smoke model produced unexpected normalization warnings: {warnings}")
    ...
    metrics: dict[str, float | bool | str] = {
        ...
        "reference_kind": PARABOLIC_FLARE_AKABAK_REFERENCE_KIND,
        ...
        "case_topology": PARABOLIC_FLARE_AKABAK_TOPOLOGY,
    }

    return ParabolicFlareAkabakComparison(
        ...
        metrics=metrics,
        limitations=PARABOLIC_FLARE_AKABAK_LIMITATIONS,
        topology=PARABOLIC_FLARE_AKABAK_TOPOLOGY,
    )


def write_parabolic_flare_akabak_compare_outputs(
    reference_dir: str | Path,
    outdir: str | Path,
) -> ParabolicFlareAkabakComparison:
    ...
    csv_path = out_path / "parabolic_flare_akabak_compare.csv"
    header = (
    ...
    summary = {
        "case": "Parabolic flare Akabak-oriented smoke",
