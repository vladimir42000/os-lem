...
@dataclass(slots=True, frozen=True)
class DirectPlusBranchedRearPathContributionContract:
    """One bounded contribution contract for a direct-plus-branched-rear path.

    Supported in this patch: exactly one direct front radiator on the driver
    front node plus exactly two distinct rear mouth radiators reached through
    one branched rear path. The contract is intentionally narrow: the front
    contribution is the direct front radiator alone, while the rear
    contribution is the coherent complex sum of the two rear mouth radiators at
    the requested observation point.
    """
...
@dataclass(slots=True, frozen=True)
class DirectPlusSplitMergeRearPathSkeleton:
    """One bounded direct-front plus split/merge rear-path skeleton.

    Supported in this patch: exactly one direct front radiator on the driver
    front node plus one minimal split/merge rear horn path from the driver rear
    node through one split junction, two bounded interior legs, one merge
    junction, and one shared rear mouth radiator.

    This keeps the opening narrow and explicit. It is not a general graph
    framework; it only records one first direct-plus-split-merge-rear topology
    built from the already landed direct-front and split/merge rear motifs.
    """
...
@dataclass(slots=True, frozen=True)
class DirectPlusBranchedSplitMergeRearPathSkeleton:
    """One bounded direct-front plus branched-split-merge rear-path skeleton.

    Supported in this patch: exactly one direct front radiator on the driver
    front node plus one rear path from the driver rear node into a three-branch
    junction where one leg terminates at a direct rear mouth radiator and the
    other leg feeds one bounded split/merge subpath that recombines into one
    second rear mouth radiator.

    This keeps the opening narrow and explicit. It is not a general graph
    framework; it only records one first direct-plus-branched-split-merge rear
    topology built from the already landed direct-front, branched, and
    split/merge rear motifs.
    """
...
@dataclass(slots=True, frozen=True)
class DirectPlusBranchedSplitMergeRearPathFrontChamberThroatSideCouplingSkeleton:
    """One bounded direct-front plus branched-split-merge rear path with front-chamber throat-side coupling.

    Supported in this patch: exactly one direct front radiator and one front
    chamber volume on the driver front node, one rear stem from the driver rear
    node into a three-way junction, one direct rear exit branch to a dedicated
    rear mouth radiator, one throat-side feed branch from that junction into a
    distinct throat-side node, one front-coupling branch from the front node
    into that same throat-side node, one downstream feed branch from the
    throat-side node into a split node, then one bounded split/merge subpath
    that recombines into one second rear mouth radiator.

    This keeps the opening narrow and explicit. It is not a general graph
    framework; it only records one first direct-plus-branched-split-merge rear
    path variant with front-chamber throat-side coupling built from already
    landed reusable motifs.
    """
...
@dataclass(slots=True, frozen=True)
class FrontRearRadiationContributionObservability:
    """One bounded front-vs-rear radiation contribution carrier.

    Supported in this patch: exactly one distinct direct-front radiator plus
    one distinct rear mouth radiator carried by one already assembled dual-
    radiator topology. The purpose is narrowly bounded: expose the individual
    front and rear radiated contributions as first-class reusable observability
    handles while preserving the existing summed-radiation path.
    """
...
class AssembledSystem:
    ...
    front_rear_radiation_contribution_observabilities: tuple[FrontRearRadiationContributionObservability, ...]
...
def _collect_direct_plus_branched_rear_path_contribution_contracts(
    *,
    direct_plus_branched_rear_path_skeletons: tuple[DirectPlusBranchedRearPathSkeleton, ...],
) -> tuple[DirectPlusBranchedRearPathContributionContract, ...]:
    contracts: list[DirectPlusBranchedRearPathContributionContract] = []
    for skeleton in direct_plus_branched_rear_path_skeletons:
        if skeleton.front_node in skeleton.rear_mouth_nodes:
            continue
        if skeleton.front_radiator_id in skeleton.rear_mouth_radiator_ids:
            continue
        if len(set(skeleton.rear_mouth_radiator_ids)) != 2:
            continue
        ...
        contracts.append(
            DirectPlusBranchedRearPathContributionContract(
                ...
                rear_branch_element_ids=skeleton.rear_branch_element_ids,
                rear_branch_element_kinds=skeleton.rear_branch_element_kinds,
                rear_mouth_nodes=skeleton.rear_mouth_nodes,
                rear_mouth_node_names=skeleton.rear_mouth_node_names,
                rear_mouth_radiator_ids=skeleton.rear_mouth_radiator_ids,
            )
        )
...
def _collect_direct_plus_split_merge_rear_path_skeletons(
    *,
    driver_front_index: int,
    node_order: tuple[str, ...],
    shunt_elements: list[AssembledElement],
    split_merge_horn_skeletons: tuple[SplitMergeHornSkeleton, ...],
) -> tuple[DirectPlusSplitMergeRearPathSkeleton, ...]:
    radiators_by_node: dict[int, list[AssembledElement]] = {}
    for element in shunt_elements:
        if element.kind == "radiator":
            radiators_by_node.setdefault(element.node_a, []).append(element)
    ...
    front_radiator = front_radiators[0]
    skeletons: list[DirectPlusSplitMergeRearPathSkeleton] = []
    for skeleton in split_merge_horn_skeletons:
        if driver_front_index == skeleton.mouth_node:
            continue
        if front_radiator.id == skeleton.mouth_radiator_id:
            continue

        skeletons.append(
            DirectPlusSplitMergeRearPathSkeleton(
                front_node=driver_front_index,
                front_node_name=node_order[driver_front_index],
                front_radiator_id=front_radiator.id,
                rear_node=skeleton.rear_node,
                ...
                stem_element_id=skeleton.stem_element_id,
                stem_element_kind=skeleton.stem_element_kind,
                rear_leg_element_ids=skeleton.leg_element_ids,
                rear_leg_element_kinds=skeleton.leg_element_kinds,
                ...
                rear_mouth_radiator_id=skeleton.mouth_radiator_id,
            )
        )
...
def _collect_direct_plus_branched_split_merge_rear_path_skeletons(
    *,
    driver_front_index: int,
    driver_rear_index: int,
    node_order: tuple[str, ...],
    ...
    acoustic_junctions: tuple[AcousticJunction, ...],
    recombination_topologies: tuple[RecombinationTopology, ...],
) -> tuple[DirectPlusBranchedSplitMergeRearPathSkeleton, ...]:
    branch_by_id = {element.id: element for element in branch_elements}
    ...
    recombinations_by_split: dict[int, list[RecombinationTopology]] = {}
    for topology in recombination_topologies:
        recombinations_by_split.setdefault(topology.split_node, []).append(topology)

    skeletons: list[DirectPlusBranchedSplitMergeRearPathSkeleton] = []
    ...
    for junction in acoustic_junctions:
        ...
        stem = stem_candidates[0]
        branch_candidates = [element for element in incident_elements if element.id != stem.id]
        if len(branch_candidates) != 2:
            continue

        for direct_branch, split_feed in (
            (branch_candidates[0], branch_candidates[1]),
            (branch_candidates[1], branch_candidates[0]),
        ):
            direct_mouth_node = _other_branch_node(direct_branch, junction.node)
            if len(branch_incidence.get(direct_mouth_node, [])) != 1:
                continue

            direct_mouth_radiators = radiators_by_node.get(direct_mouth_node, [])
            if len(direct_mouth_radiators) != 1:
                continue

            split_node = _other_branch_node(split_feed, junction.node)
            candidate_recombinations = recombinations_by_split.get(split_node, [])
            if len(candidate_recombinations) != 1:
                continue

            recombination = candidate_recombinations[0]
            if direct_mouth_node == recombination.mouth_node:
                continue
            if direct_mouth_radiators[0].id == recombination.mouth_radiator_id:
                continue
            if driver_front_index in (direct_mouth_node, recombination.mouth_node):
                continue
            if front_radiator.id in (direct_mouth_radiators[0].id, recombination.mouth_radiator_id):
                continue

            skeletons.append(
                DirectPlusBranchedSplitMergeRearPathSkeleton(
                    front_node=driver_front_index,
                    front_node_name=node_order[driver_front_index],
                    front_radiator_id=front_radiator.id,
                    rear_node=driver_rear_index,
                    ...
                    stem_element_id=stem.id,
                    stem_element_kind=stem.kind,
                    direct_branch_element_id=direct_branch.id,
                    direct_branch_element_kind=direct_branch.kind,
                    direct_rear_mouth_node=direct_mouth_node,
                    direct_rear_mouth_node_name=node_order[direct_mouth_node],
                    direct_rear_mouth_radiator_id=direct_mouth_radiators[0].id,
                    split_feed_element_id=split_feed.id,
                    split_feed_element_kind=split_feed.kind,
                    split_node=recombination.split_node,
                    split_node_name=recombination.split_node_name,
                    merge_node=recombination.merge_node,
                    merge_node_name=recombination.merge_node_name,
                    recombined_leg_element_ids=recombination.upstream_bundle_element_ids,
                    recombined_leg_element_kinds=recombination.upstream_bundle_element_kinds,
                    shared_exit_element_id=recombination.shared_exit_element_id,
                    shared_exit_element_kind=recombination.shared_exit_element_kind,
                    merged_rear_mouth_node=recombination.mouth_node,
                    merged_rear_mouth_node_name=recombination.mouth_node_name,
                    merged_rear_mouth_radiator_id=recombination.mouth_radiator_id,
                )
            )
            break
...
def _collect_direct_plus_branched_split_merge_rear_path_front_chamber_throat_side_coupling_skeletons(
    *,
    driver_front_index: int,
    driver_rear_index: int,
    node_order: tuple[str, ...],
    ...
    acoustic_junctions: tuple[AcousticJunction, ...],
    recombination_topologies: tuple[RecombinationTopology, ...],
) -> tuple[DirectPlusBranchedSplitMergeRearPathFrontChamberThroatSideCouplingSkeleton, ...]:
    branch_by_id = {element.id: element for element in branch_elements}
    junction_by_node = {junction.node: junction for junction in acoustic_junctions}
    ...
    if len(front_radiators) != 1 or len(front_volumes) != 1:
        return ()

    front_radiator = front_radiators[0]
    front_volume = front_volumes[0]
    ...
    skeletons: list[DirectPlusBranchedSplitMergeRearPathFrontChamberThroatSideCouplingSkeleton] = []

    throat_side_node = _other_branch_node(front_coupling, driver_front_index)
    if throat_side_node in (driver_front_index, driver_rear_index):
        return ()
    if radiators_by_node.get(throat_side_node) or volumes_by_node.get(throat_side_node):
        return ()

    throat_side_incident = branch_incidence.get(throat_side_node, [])
    if len(throat_side_incident) != 3:
        return ()

    throat_side_candidates = [element for element in throat_side_incident if element.id != front_coupling.id]
    if len(throat_side_candidates) != 2:
        return ()

    for throat_feed_upstream, throat_feed_downstream in (
        (throat_side_candidates[0], throat_side_candidates[1]),
        (throat_side_candidates[1], throat_side_candidates[0]),
    ):
        junction_node = _other_branch_node(throat_feed_upstream, throat_side_node)
        if junction_node == driver_front_index:
            continue
        junction = junction_by_node.get(junction_node)
        if junction is None or len(junction.incident_element_ids) != 3:
            continue

        junction_incident = [branch_by_id[element_id] for element_id in junction.incident_element_ids]
        if throat_feed_upstream.id not in junction.incident_element_ids:
            continue

        stem_candidates = [
            element
            for element in junction_incident
            if _other_branch_node(element, junction_node) == driver_rear_index
        ]
        if len(stem_candidates) != 1:
            continue
        stem = stem_candidates[0]
        ...
        direct_candidates = [
            element
            for element in junction_incident
            if element.id not in {stem.id, throat_feed_upstream.id}
        ]
        if len(direct_candidates) != 1:
            continue
        direct_branch = direct_candidates[0]

        direct_mouth_node = _other_branch_node(direct_branch, junction_node)
        if len(branch_incidence.get(direct_mouth_node, [])) != 1:
            continue
        direct_mouth_radiators = radiators_by_node.get(direct_mouth_node, [])
        if len(direct_mouth_radiators) != 1:
        ...
        split_node = _other_branch_node(throat_feed_downstream, throat_side_node)
        candidate_recombinations = recombinations_by_split.get(split_node, [])
        if len(candidate_recombinations) != 1:
            continue
        recombination = candidate_recombinations[0]
        ...
        if driver_front_index in (direct_mouth_node, recombination.mouth_node):
            continue
        if throat_side_node in (direct_mouth_node, recombination.mouth_node):
            continue
        if direct_mouth_node == recombination.mouth_node:
            continue
        if front_radiator.id in (direct_mouth_radiators[0].id, recombination.mouth_radiator_id):
        ...
        skeletons.append(
            DirectPlusBranchedSplitMergeRearPathFrontChamberThroatSideCouplingSkeleton(
                front_node=driver_front_index,
                front_node_name=node_order[driver_front_index],
                front_radiator_id=front_radiator.id,
                front_chamber_element_id=front_volume.id,
                front_chamber_element_kind=front_volume.kind,
                rear_node=driver_rear_index,
                rear_node_name=node_order[driver_rear_index],
                junction_node=junction_node,
                junction_node_name=node_order[junction_node],
                stem_element_id=stem.id,
                stem_element_kind=stem.kind,
                direct_branch_element_id=direct_branch.id,
                direct_branch_element_kind=direct_branch.kind,
                ...
                throat_feed_upstream_element_id=throat_feed_upstream.id,
                throat_feed_upstream_element_kind=throat_feed_upstream.kind,
                throat_feed_downstream_element_id=throat_feed_downstream.id,
                throat_feed_downstream_element_kind=throat_feed_downstream.kind,
...
def _collect_front_rear_radiation_contribution_observabilities(
    *,
    front_rear_radiation_sum_observabilities: tuple[FrontRearRadiationSumObservability, ...],
) -> tuple[FrontRearRadiationContributionObservability, ...]:
    observabilities: list[FrontRearRadiationContributionObservability] = []
    for observability in front_rear_radiation_sum_observabilities:
        observabilities.append(
            FrontRearRadiationContributionObservability(
                front_node=observability.front_node,
                front_node_name=observability.front_node_name,
                front_radiator_id=observability.front_radiator_id,
                rear_mouth_node=observability.rear_mouth_node,
                rear_mouth_node_name=observability.rear_mouth_node_name,
                rear_radiator_id=observability.rear_radiator_id,
            )
        )
...
def assemble_system(model: NormalizedModel) -> AssembledSystem:
    ...
    direct_plus_branched_rear_path_contribution_contracts = _collect_direct_plus_branched_rear_path_contribution_contracts(
    ...
    direct_plus_split_merge_rear_path_skeletons = _collect_direct_plus_split_merge_rear_path_skeletons(
        driver_front_index=driver_front_index,
        node_order=node_order,
        shunt_elements=shunt_elements,
    ...
    direct_plus_branched_split_merge_rear_path_skeletons = _collect_direct_plus_branched_split_merge_rear_path_skeletons(
        driver_front_index=driver_front_index,
        driver_rear_index=driver_rear_index,
        node_order=node_order,
        shunt_elements=shunt_elements,
    ...
    direct_plus_branched_split_merge_rear_path_front_chamber_throat_side_coupling_skeletons = _collect_direct_plus_branched_split_merge_rear_path_front_chamber_throat_side_coupling_skeletons(
        driver_front_index=driver_front_index,
        driver_rear_index=driver_rear_index,
        node_order=node_order,
        shunt_elements=shunt_elements,
    ...
    front_rear_radiation_contribution_observabilities = _collect_front_rear_radiation_contribution_observabilities(
    ...
    return AssembledSystem(
        ...
        dual_radiator_topologies=dual_radiator_topologies,
        front_rear_radiation_sum_observabilities=front_rear_radiation_sum_observabilities,
        front_rear_radiation_contribution_observabilities=front_rear_radiation_contribution_observabilities,
