...
from .reference_tractrix_flare_smoke import tractrix_model_dict
from .solve import radiator_observation_pressure, solve_frequency_sweep

TRACTRIX_FLARE_AKABAK_TOPOLOGY = (
    "one rear volume behind the driver, one tractrix-only rear-to-mouth flare path, "
    "one direct front radiator, and one mouth radiator"
)

TRACTRIX_FLARE_AKABAK_REFERENCE_KIND = "akabak_oriented_segmented_conical_surrogate"

TRACTRIX_FLARE_AKABAK_LIMITATIONS = (
    "This bundle is an Akabak-oriented smoke surface for the tractrix-only flare path: the reference files use "
    ...
    "The bundled reference script approximates the same tractrix flare with a fixed 12-section conical chain because "
...
@dataclass(slots=True, frozen=True)
class TractrixFlareAkabakReferenceBundle:
...
@dataclass(slots=True, frozen=True)
class TractrixFlareAkabakComparison:
    frequency_hz: np.ndarray
...
def load_tractrix_flare_akabak_reference_bundle(
    reference_dir: str | Path,
) -> TractrixFlareAkabakReferenceBundle:
    ref_dir = Path(reference_dir)
    impedance = _load_two_column_series(
        ref_dir / "TRX_Z.ZMA",
        unit="ohm",
        label="Akabak-oriented tractrix-flare input impedance magnitude",
    )
    pressure_db = _load_two_column_series(
        ref_dir / "TRX_LP.FRD",
        unit="dB",
        label="Akabak-oriented tractrix-flare mouth pressure level",
    )
    pressure_pa = _load_two_column_series(
        ref_dir / "TRX_AMP.FRD",
        unit="Pa",
        label="Akabak-oriented tractrix-flare mouth pressure amplitude",
    )
    pressure_phase = _load_two_column_series(
        ref_dir / "TRX_PHS.FRD",
        unit="deg",
        label="Akabak-oriented tractrix-flare mouth pressure phase",
    )
    ...
    if not (
        ...
        raise ValueError("Tractrix flare reference files do not share an identical frequency grid")

    script_text = (ref_dir / "TRX.aks").read_text(encoding="latin1")
    return TractrixFlareAkabakReferenceBundle(
...
def build_tractrix_flare_model_dict() -> dict[str, Any]:
    model_dict = copy.deepcopy(tractrix_model_dict())
    model_dict["meta"] = {
        "name": "tractrix_flare_akabak_oriented_smoke",
        "radiation_space": "2pi",
    }
...
def build_tractrix_flare_segmented_conical_reference_model_dict() -> dict[str, Any]:
    model_dict = build_tractrix_flare_model_dict()
    ...
    for idx in range(segments):
        conical_segments.append(
            {
                "id": f"trx_seg_{idx + 1:02d}",
                "type": "waveguide_1d",
                "node_a": "rear" if idx == 0 else f"trx_node_{idx}",
                "node_b": "mouth" if idx == segments - 1 else f"trx_node_{idx + 1}",
    ...
    model_dict["meta"]["name"] = "tractrix_flare_akabak_segmented_conical_reference"
    return model_dict
...
def compare_tractrix_flare_against_akabak_oriented_reference(
    reference_dir: str | Path,
) -> TractrixFlareAkabakComparison:
    bundle = load_tractrix_flare_akabak_reference_bundle(reference_dir)
    model, warnings = normalize_model(build_tractrix_flare_model_dict())
    if warnings:
        raise ValueError(f"Tractrix flare smoke model produced unexpected normalization warnings: {warnings}")
    ...
    metrics: dict[str, float | bool | str] = {
        ...
        "reference_kind": TRACTRIX_FLARE_AKABAK_REFERENCE_KIND,
        ...
        "case_topology": TRACTRIX_FLARE_AKABAK_TOPOLOGY,
    }

    return TractrixFlareAkabakComparison(
        ...
        metrics=metrics,
        limitations=TRACTRIX_FLARE_AKABAK_LIMITATIONS,
        topology=TRACTRIX_FLARE_AKABAK_TOPOLOGY,
    )


def write_tractrix_flare_akabak_compare_outputs(
    reference_dir: str | Path,
    outdir: str | Path,
) -> TractrixFlareAkabakComparison:
    comparison = compare_tractrix_flare_against_akabak_oriented_reference(reference_dir)
    out_path = Path(outdir)
    out_path.mkdir(parents=True, exist_ok=True)

    csv_path = out_path / "tractrix_flare_akabak_compare.csv"
    header = (
    ...
    summary = {
        "case": "Tractrix flare Akabak-oriented smoke",
