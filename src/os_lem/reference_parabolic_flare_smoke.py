...
class ParabolicSmokeObservation:
    segments: int
    x_m: np.ndarray
...
@dataclass(frozen=True)
class ParabolicSmokeReport:
    observations: tuple[ParabolicSmokeObservation, ...]
    frequency_hz: float


def parabolic_model_dict() -> dict:
    return {
        "driver": {
            "id": "drv1",
            "model": "ts_classic",
...
def build_parabolic_model_with_segments(segments: int) -> dict:
    ...
    for element in model_dict["elements"]:
        if element.get("id") == "wg1":
            ...
            return model_dict
    raise AssertionError("expected wg1 element in parabolic smoke model")
...
def evaluate_parabolic_reference_smoke(
    ...
    *,
    ...
    node_a: str = "rear",
    node_b: str = "mouth",
) -> ParabolicSmokeReport:
    observations: list[ParabolicSmokeObservation] = []
    ...
    return ParabolicSmokeReport(observations=tuple(observations), frequency_hz=float(frequency_hz))
