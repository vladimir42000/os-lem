...
from .reference_exponential_flare_smoke import exponential_model_dict
from .solve import radiator_observation_pressure, solve_frequency_sweep

EXPONENTIAL_FLARE_AKABAK_TOPOLOGY = (
    "one rear volume behind the driver, one exponential-only rear-to-mouth flare path, "
    "one direct front radiator, and one mouth radiator"
)

EXPONENTIAL_FLARE_AKABAK_REFERENCE_KIND = "akabak_oriented_segmented_conical_surrogate"

EXPONENTIAL_FLARE_AKABAK_LIMITATIONS = (
    "This bundle is an Akabak-oriented smoke surface for the exponential-only flare path: the reference files use "
    ...
    "The bundled reference script approximates the same exponential flare with a fixed 12-section conical chain because "
...
@dataclass(slots=True, frozen=True)
class ExponentialFlareAkabakReferenceBundle:
...
@dataclass(slots=True, frozen=True)
class ExponentialFlareAkabakComparison:
    frequency_hz: np.ndarray
...
def load_exponential_flare_akabak_reference_bundle(
    reference_dir: str | Path,
) -> ExponentialFlareAkabakReferenceBundle:
    ref_dir = Path(reference_dir)
    impedance = _load_two_column_series(
        ref_dir / "EXP_Z.ZMA",
        unit="ohm",
        label="Akabak-oriented exponential-flare input impedance magnitude",
    )
    pressure_db = _load_two_column_series(
        ref_dir / "EXP_LP.FRD",
        unit="dB",
        label="Akabak-oriented exponential-flare mouth pressure level",
    )
    pressure_pa = _load_two_column_series(
        ref_dir / "EXP_AMP.FRD",
        unit="Pa",
        label="Akabak-oriented exponential-flare mouth pressure amplitude",
    )
    pressure_phase = _load_two_column_series(
        ref_dir / "EXP_PHS.FRD",
        unit="deg",
        label="Akabak-oriented exponential-flare mouth pressure phase",
    )
    ...
    if not (
        ...
        raise ValueError("Exponential flare reference files do not share an identical frequency grid")

    script_text = (ref_dir / "EXP.aks").read_text(encoding="latin1")
    return ExponentialFlareAkabakReferenceBundle(
...
def build_exponential_flare_model_dict() -> dict[str, Any]:
    model_dict = copy.deepcopy(exponential_model_dict())
    model_dict["meta"] = {
        "name": "exponential_flare_akabak_oriented_smoke",
        "radiation_space": "2pi",
    }
...
def build_exponential_flare_segmented_conical_reference_model_dict() -> dict[str, Any]:
    ...
    for idx in range(segments):
        conical_segments.append(
            {
                "id": f"exp_seg_{idx + 1:02d}",
                "type": "waveguide_1d",
                "node_a": "rear" if idx == 0 else f"exp_node_{idx}",
                "node_b": "mouth" if idx == segments - 1 else f"exp_node_{idx + 1}",
    ...
    model_dict["meta"]["name"] = "exponential_flare_akabak_segmented_conical_reference"
    return model_dict
...
def compare_exponential_flare_against_akabak_oriented_reference(
    reference_dir: str | Path,
) -> ExponentialFlareAkabakComparison:
    bundle = load_exponential_flare_akabak_reference_bundle(reference_dir)
    model, warnings = normalize_model(build_exponential_flare_model_dict())
    if warnings:
        raise ValueError(f"Exponential flare smoke model produced unexpected normalization warnings: {warnings}")
    ...
    metrics: dict[str, float | bool | str] = {
        ...
        "reference_kind": EXPONENTIAL_FLARE_AKABAK_REFERENCE_KIND,
        ...
        "case_topology": EXPONENTIAL_FLARE_AKABAK_TOPOLOGY,
    }

    return ExponentialFlareAkabakComparison(
        ...
        metrics=metrics,
        limitations=EXPONENTIAL_FLARE_AKABAK_LIMITATIONS,
        topology=EXPONENTIAL_FLARE_AKABAK_TOPOLOGY,
    )


def write_exponential_flare_akabak_compare_outputs(
    reference_dir: str | Path,
    outdir: str | Path,
) -> ExponentialFlareAkabakComparison:
    ...
    csv_path = out_path / "exponential_flare_akabak_compare.csv"
    header = (
    ...
    summary = {
        "case": "Exponential flare Akabak-oriented smoke",
