...
def test_parser_accepts_lecleach_named_flare_contract() -> None:
    model, _ = normalize_model(_base_model("lecleach"))
    assert model.waveguides[0].profile == "lecleach"


@pytest.mark.parametrize("bad_profile", ["Le Cleac'h", "le cleach", "", 123])
def test_parser_rejects_non_contract_named_flare_tokens_after_lecleach_opening(bad_profile) -> None:
