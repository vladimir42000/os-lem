...
def _driver_front_chamber_model(*, include_front_volume: bool = True) -> NormalizedModel:
    volumes = [
...
def test_assemble_system_collects_one_driver_front_chamber_topology() -> None:
    assembled = assemble_system(_driver_front_chamber_model())

    assert len(assembled.throat_chamber_topologies) == 0
    assert len(assembled.blind_throat_side_segment_topologies) == 0
    assert len(assembled.driver_front_chamber_topologies) == 1

    topology = assembled.driver_front_chamber_topologies[0]
...
def test_assemble_system_does_not_claim_driver_front_chamber_topology_without_front_volume() -> None:
    assembled = assemble_system(_driver_front_chamber_model(include_front_volume=False))
