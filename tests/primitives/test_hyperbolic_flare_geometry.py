...
def test_hyperbolic_area_progression_is_monotone_and_endpoint_preserving() -> None:
    length_m = 0.4
    a0 = 10e-4
    a1 = 50e-4
    xs = np.linspace(0.0, length_m, 11)
    areas = np.array([area_at_position(length_m, a0, a1, float(x), profile="hyperbolic") for x in xs])
    np.testing.assert_allclose(areas[0], a0)
    np.testing.assert_allclose(areas[-1], a1)
    assert np.all(np.diff(areas) > 0.0)


def test_hyperbolic_midpoint_areas_differ_from_current_other_named_laws() -> None:
    length_m = 0.4
    a0 = 10e-4
    a1 = 50e-4
    ...
    assert np.max(np.abs(hyperbolic_mid - conical_mid)) > 0.0
    assert np.max(np.abs(hyperbolic_mid - exponential_mid)) > 0.0
    assert np.max(np.abs(hyperbolic_mid - tractrix_mid)) > 0.0
