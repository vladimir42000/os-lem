...
def _rear_chamber_port_injection_model_dict() -> dict[str, object]:
    return {
        "meta": {"name": "rear_chamber_port_injection_demo", "radiation_space": "2pi"},
        "driver": {
            "id": "drv1",
            "model": "ts_classic",
            "Re": "5.8 ohm",
...
def test_run_simulation_supports_rear_chamber_port_injection_topology_end_to_end() -> None:
    model_dict = _rear_chamber_port_injection_model_dict()
    ...
    assert result.units["p_inject"] == "Pa"
    ...
    np.testing.assert_allclose(result.series["rear_port_q_b"], result.series["stem_q_a"], rtol=1e-10, atol=1e-12)
    ...
    assert not np.allclose(result.series["p_inject"], result.series["p_tap"])
