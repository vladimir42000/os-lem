...
def _driver_front_chamber_model_dict() -> dict[str, object]:
    return {
        "meta": {"name": "driver_front_chamber_demo", "radiation_space": "2pi"},
        "driver": {
            "id": "drv1",
            "model": "ts_classic",
            "Re": "5.8 ohm",
        ...
        "elements": [
            ...
            {"id": "blind_segment", "type": "waveguide_1d", "node_a": "throat", "node_b": "blind", "length": "9 cm", "area_start": "48 cm2", "area_end": "39 cm2", "profile": "conical", "segments": 2},
            ...
            {"id": "front_coupling", "type": "waveguide_1d", "node_a": "front", "node_b": "tap", "length": "5 cm", "area_start": "34 cm2", "area_end": "36 cm2", "profile": "conical", "segments": 2},
...
def test_run_simulation_supports_driver_front_chamber_topology_end_to_end() -> None:
    model_dict = _driver_front_chamber_model_dict()
    ...
    assert np.all(np.isfinite(result.series["p_tap"].imag))

    assert np.all(np.abs(result.series["blind_q_b"]) < 1.0e-10)
    np.testing.assert_allclose(
        result.series["tap_down_q_a"],
        result.series["tap_up_q_b"] + result.series["front_q_b"],
        rtol=1e-10,
        atol=1e-12,
    )
    np.testing.assert_allclose(
    ...
    np.testing.assert_allclose(result.series["exit_q_a"] * 0 + result.series["mouth_q"], result.series["mouth_q"], rtol=1e-10, atol=1e-12)
    assert not np.allclose(result.series["p_front"], result.series["p_tap"])
    assert not np.allclose(result.series["p_throat"], result.series["p_front"])
