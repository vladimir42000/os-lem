...
def _offset_tap_model_dict() -> dict[str, object]:
    return {
        "meta": {"name": "offset_tap_demo", "radiation_space": "2pi"},
        "driver": {
            "id": "drv1",
            "model": "ts_classic",
            "Re": "5.8 ohm",
...
def test_run_simulation_supports_offset_tap_topology_end_to_end() -> None:
    model_dict = _offset_tap_model_dict()
    ...
    assert np.all(np.isfinite(result.series["p_tap"].real))
    ...
    assert not np.allclose(result.series["p_tap"], result.series["p_merge"])
