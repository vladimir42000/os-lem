...
def _blind_throat_model_dict() -> dict[str, object]:
    return {
        "meta": {"name": "blind_throat_demo", "radiation_space": "2pi"},
        "driver": {
            "id": "drv1",
            "model": "ts_classic",
            "Re": "5.8 ohm",
...
def test_run_simulation_supports_blind_throat_side_segment_topology_end_to_end() -> None:
    model_dict = _blind_throat_model_dict()
    ...
    assert result.units["p_blind"] == "Pa"
    assert np.all(np.isfinite(result.zin_mag_ohm))
    assert np.all(np.isfinite(result.series["p_throat"].real))
    assert np.all(np.isfinite(result.series["p_blind"].imag))

    np.testing.assert_allclose(result.series["rear_port_q_b"], result.series["throat_entry_q_a"], rtol=1e-10, atol=1e-12)
    np.testing.assert_allclose(result.series["blind_q_b"], 0.0, rtol=1e-10, atol=1e-12)
    ...
    assert not np.allclose(result.series["p_throat"], result.series["p_blind"])
