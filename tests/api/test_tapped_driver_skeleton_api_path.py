...
def _tapped_driver_model_dict() -> dict[str, object]:
    return {
        "meta": {"name": "tapped_driver_demo", "radiation_space": "2pi"},
        "driver": {
            "id": "drv1",
            "model": "ts_classic",
            "Re": "5.8 ohm",
        ...
        "observations": [
            ...
            {"id": "u_cone", "type": "cone_velocity", "target": "drv1"},
...
def test_run_simulation_supports_tapped_driver_skeleton_end_to_end() -> None:
    model_dict = _tapped_driver_model_dict()
    ...
    assert result.units["u_cone"] == "m/s"
    assert result.units["p_merge"] == "Pa"
    assert np.all(np.isfinite(result.zin_mag_ohm))
    assert np.all(np.isfinite(result.series["u_cone"]))
    ...
    np.testing.assert_allclose(
        ...
        result.series["main_q_b"] + result.series["tap_q_b"] + normalized.driver.Sd * result.series["u_cone"],
        rtol=1e-10,
        atol=1e-12,
    )
