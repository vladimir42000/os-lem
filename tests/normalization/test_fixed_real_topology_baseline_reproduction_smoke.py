...
_FREQUENCIES_HZ = np.array([30.0, 40.0, 60.0, 80.0, 120.0, 160.0], dtype=float)
...
def _packet_to_model_dict(packet: Mapping[str, Any]) -> dict[str, Any]:
    """Bounded packet -> model_dict helper used only by this smoke test.

    This helper is intentionally narrow. It supports the accepted fixed
    normalization packet shape and one truthful offset-line / TQWT-family model
    construction under the existing segmented 1D kernel discipline.
    """
    ...
    return {
        "meta": {
            "name": "fixed_real_topology_baseline_reproduction_smoke",
            "radiation_space": "2pi",
        },
        "driver": {
            "id": "drv1",
...
def test_fixed_real_topology_adapter_drives_solver_baseline_reproduction_smoke() -> None:
    ...
    result_a = run_simulation(model_dict, _FREQUENCIES_HZ)
    result_b = run_simulation(model_dict, _FREQUENCIES_HZ)

    assert result_a.zin_complex_ohm is not None
    assert result_a.zin_mag_ohm is not None

    spl_total_a = np.asarray(result_a.series["spl_total"], dtype=float)
    spl_total_b = np.asarray(result_b.series["spl_total"], dtype=float)
    zin_a = np.asarray(result_a.zin_complex_ohm)
    zin_b = np.asarray(result_b.zin_complex_ohm)

    assert result_a.frequencies_hz.shape == _FREQUENCIES_HZ.shape
    assert spl_total_a.shape == _FREQUENCIES_HZ.shape
    assert zin_a.shape == _FREQUENCIES_HZ.shape

    assert np.all(np.isfinite(result_a.frequencies_hz))
    assert np.all(np.isfinite(spl_total_a))
    assert np.all(np.isfinite(np.real(zin_a)))
    assert np.all(np.isfinite(np.imag(zin_a)))

    np.testing.assert_allclose(result_a.frequencies_hz, _FREQUENCIES_HZ, rtol=0.0, atol=0.0)
    np.testing.assert_allclose(spl_total_a, spl_total_b, rtol=1.0e-12, atol=1.0e-12)
    np.testing.assert_allclose(zin_a, zin_b, rtol=1.0e-12, atol=1.0e-12)

    assert not np.allclose(spl_total_a, 0.0, atol=1.0e-9)
    assert np.max(np.abs(spl_total_a)) > 1.0
