...
def _rear_chamber_tapped_model() -> NormalizedModel:
...
def test_solve_frequency_point_supports_rear_chamber_tapped_skeleton() -> None:
    model = _rear_chamber_tapped_model()
    ...
    rear_index = solved.node_order.index("rear")
    tap_index = solved.node_order.index("tap")
    assert not np.isclose(solved.pressures[rear_index], solved.pressures[tap_index])
