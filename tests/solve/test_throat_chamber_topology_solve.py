...
def _throat_chamber_model() -> NormalizedModel:
...
def test_solve_frequency_point_supports_throat_chamber_topology() -> None:
    model = _throat_chamber_model()
    system = assemble_system(model)

    solved = solve_frequency_point(model, system, 160.0)

    assert len(system.throat_chamber_topologies) == 1
    assert solved.node_order == ("rear", "inject", "throat", "split", "tap", "merge", "mouth")
    ...
    throat_entry_into_throat = -solved.waveguide_endpoint_flow["throat_entry"].node_b
    stem_from_throat = solved.waveguide_endpoint_flow["stem"].node_a
    ...
    assert abs(throat_entry_into_throat) > abs(stem_from_throat)
    ...
    assert not np.isclose(solved.pressures[throat_index], solved.pressures[inject_index])
