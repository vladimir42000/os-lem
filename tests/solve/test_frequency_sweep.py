...
def _minimal_waveguide_cylindrical_lossy_model(loss_np_per_m: float) -> NormalizedModel:
    return NormalizedModel(
        driver=_driver(),
        volumes=[
            VolumeElement(id="rear_vol", node="rear", value_m3=0.02),
...
def _exact_cylindrical_losssy_admittance_matrix(omega: float, length_m: float, area_m2: float, loss_np_per_m: float) -> np.ndarray:
    ...
    return np.array(
        [
            [(1.0 / zc_a) * (c / s), -(1.0 / zc_a) * (1.0 / s)],
            [-(1.0 / zc_a) * (1.0 / s), (1.0 / zc_a) * (c / s)],
        ],
        dtype=np.complex128,
    )


def _exact_cylindrical_profile_from_endpoints(
    omega: float,
    length_m: float,
    area_m2: float,
    loss_np_per_m: float,
    pressure_left: complex,
    pressure_right: complex,
    x_m: np.ndarray,
    ...
    sinh_gl = np.sinh(gl)
    pressure = (
        np.sinh(gamma * (length_m - x_m)) / sinh_gl * pressure_left
        + np.sinh(gamma * x_m) / sinh_gl * pressure_right
    )
    volume_velocity = -(
        1.0 / (zc_a * sinh_gl)
    ) * (
        -pressure_left * np.cosh(gamma * (length_m - x_m))
        + pressure_right * np.cosh(gamma * x_m)
    )
    particle_velocity = volume_velocity / area_m2
    return pressure, volume_velocity, particle_velocity


def test_loss_zero_reproduces_current_lossless_cylindrical_waveguide_behavior() -> None:
    model_lossless = _minimal_waveguide_cylindrical_model()
    system_lossless = assemble_system(model_lossless)
    point_lossless = solve_frequency_point(model_lossless, system_lossless, 100.0)

    model_zero_loss = _minimal_waveguide_cylindrical_lossy_model(0.0)
...
def test_lossy_cylindrical_waveguide_equivalent_admittance_matches_exact_reference() -> None:
    model = _minimal_waveguide_cylindrical_lossy_model(0.35)
    waveguide = model.waveguides[0]
    omega = 2.0 * np.pi * 100.0

    actual = _waveguide_equivalent_admittance_matrix(omega, waveguide)
    expected = _exact_cylindrical_losssy_admittance_matrix(
        omega,
        waveguide.length_m,
        waveguide.area_start_m2,
        waveguide.loss or 0.0,
    )

    np.testing.assert_allclose(actual, expected)


def test_lossy_cylindrical_profiles_match_exact_reference_from_solved_endpoints() -> None:
    model = _minimal_waveguide_cylindrical_lossy_model(0.35)
    system = assemble_system(model)
    point = solve_frequency_point(model, system, 100.0)
    ...
    waveguide = model.waveguides[0]
    expected_p, expected_q, expected_v = _exact_cylindrical_profile_from_endpoints(
        point.omega_rad_s,
        waveguide.length_m,
        waveguide.area_start_m2,
        waveguide.loss or 0.0,
    ...
    np.testing.assert_allclose(pressure_profile.values, expected_p)
    np.testing.assert_allclose(flow_profile.values, expected_q)
    np.testing.assert_allclose(particle_profile.values, expected_v)


def test_increasing_cylindrical_loss_reduces_transmitted_pressure_magnitude() -> None:
    model_low = _minimal_waveguide_cylindrical_lossy_model(0.05)
