...
def test_solve_frequency_point_supports_front_chamber_throat_side_coupling_topology() -> None:
    model = _model()
    ...
    assert solved.node_order == ("rear", "inject", "throat", "throat_side", "blind", "split", "front", "tap", "merge", "mouth")
    assert solved.pressures.shape == (10,)
    ...
    front_coupling_into_side = -solved.waveguide_endpoint_flow["front_coupling"].node_b
    ...
    np.testing.assert_allclose(blind_down_from_side, blind_up_into_side + front_coupling_into_side, rtol=1e-10, atol=1e-12)
    ...
    side_index = solved.node_order.index("throat_side")
    front_index = solved.node_order.index("front")
    assert not np.isclose(solved.pressures[side_index], solved.pressures[front_index])
