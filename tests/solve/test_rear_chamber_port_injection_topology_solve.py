...
def _rear_chamber_port_injection_model() -> NormalizedModel:
...
def test_solve_frequency_point_supports_rear_chamber_port_injection_topology() -> None:
    model = _rear_chamber_port_injection_model()
    system = assemble_system(model)

    solved = solve_frequency_point(model, system, 160.0)

    assert len(system.rear_chamber_port_injection_topologies) == 1
    assert solved.node_order == ("rear", "inject", "split", "tap", "merge", "mouth")
    assert solved.pressures.shape == (6,)
    ...
    stem_from_inject = solved.waveguide_endpoint_flow["stem"].node_a
    ...
    np.testing.assert_allclose(rear_port_into_inject, stem_from_inject, rtol=1e-10, atol=1e-12)
    ...
    inject_index = solved.node_order.index("inject")
    tap_index = solved.node_order.index("tap")
    assert not np.isclose(solved.pressures[inject_index], solved.pressures[tap_index])
