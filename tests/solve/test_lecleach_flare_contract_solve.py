...
def test_lecleach_waveguide_path_remains_finite_and_endpoint_profile_consistent() -> None:
    model, _ = normalize_model(_lecleach_model_dict())
...
def test_lecleach_internal_segment_boundary_flows_match_discrete_segment_semantics() -> None:
    model, _ = normalize_model(_lecleach_model_dict())
    system = assemble_system(model)
    point = solve_frequency_point(model, system, 120.0)
