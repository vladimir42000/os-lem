...
def _split_merge_model() -> NormalizedModel:
...
def test_solve_frequency_point_supports_split_merge_horn_skeleton() -> None:
    model = _split_merge_model()
    system = assemble_system(model)

    solved = solve_frequency_point(model, system, 150.0)

    assert len(system.split_merge_horn_skeletons) == 1
    assert solved.node_order == ("front", "rear", "split", "merge", "mouth")
    ...
    np.testing.assert_allclose(exit_from_merge, main_into_merge + tap_into_merge, rtol=1e-10, atol=1e-12)
