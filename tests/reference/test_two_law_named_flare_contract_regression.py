...
@pytest.mark.parametrize("profile", ["exponential", "tractrix"])
def test_two_law_named_flare_contract_accepts_the_two_current_named_laws(profile: str) -> None:
    model, _ = normalize_model(_named_flare_model(profile))
    assert model.waveguides[0].profile == profile


@pytest.mark.parametrize("bad_profile", ["hyperbolic", "parabolic", "Le Cleac'h", "", 123])
def test_two_law_named_flare_contract_still_rejects_non_contract_named_law_tokens(bad_profile) -> None:
...
def test_two_law_named_flare_contract_preserves_shared_profile_and_endpoint_observability_semantics() -> None:
    solved = {profile: _solve_shared_profile_contract(profile) for profile in ("exponential", "tractrix")}
