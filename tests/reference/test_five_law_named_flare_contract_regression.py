...
from os_lem.reference_lecleach_flare_smoke import build_lecleach_model_with_segments
...
ACCEPTED_FIVE_LAW_PROFILES = ("exponential", "tractrix", "hyperbolic", "parabolic", "lecleach")
...
def _named_flare_model(profile: str, *, segments: int = 12) -> dict:
    if profile == "exponential":
        ...
        model_dict = build_lecleach_model_with_segments(segments)
    else:
...
@pytest.mark.parametrize("profile", ACCEPTED_FIVE_LAW_PROFILES)
def test_five_law_named_flare_contract_accepts_the_five_current_named_laws(profile: str) -> None:
...
def test_five_law_named_flare_contract_still_rejects_non_contract_named_law_tokens(bad_profile) -> None:
...
def test_five_law_named_flare_contract_preserves_shared_profile_and_endpoint_observability_semantics() -> None:
    solved = {profile: _solve_shared_profile_contract(profile) for profile in ACCEPTED_FIVE_LAW_PROFILES}
    ...
    first = solved[ACCEPTED_FIVE_LAW_PROFILES[0]]
    for profile in ACCEPTED_FIVE_LAW_PROFILES[1:]:
    ...
    for i, left in enumerate(ACCEPTED_FIVE_LAW_PROFILES):
        for right in ACCEPTED_FIVE_LAW_PROFILES[i + 1 :]:
