...
from os_lem.reference_direct_plus_branched_split_merge_rear_path import (
    ...
    compare_direct_plus_branched_split_merge_rear_path_against_refined_reference,
    write_direct_plus_branched_split_merge_rear_path_compare_outputs,
)


def test_direct_plus_branched_split_merge_rear_path_smoke_model_assembles_one_supported_skeleton() -> None:
    model_dict = build_direct_plus_branched_split_merge_rear_path_model_dict()
...
def test_direct_plus_branched_split_merge_rear_path_smoke_run_simulation_preserves_flow_balances() -> None:
    frequencies_hz = np.array([60.0, 120.0, 240.0, 480.0])
    result = run_simulation(build_direct_plus_branched_split_merge_rear_path_model_dict(), frequencies_hz)
    ...
    assert np.all(np.isfinite(result.series["spl_direct"]))
    assert np.all(np.isfinite(result.series["spl_merged"]))
    ...
    assert not np.allclose(result.series["spl_direct"], result.series["spl_merged"])
...
def test_direct_plus_branched_split_merge_rear_path_smoke_compare_produces_bounded_refinement_metrics() -> None:
    comparison = compare_direct_plus_branched_split_merge_rear_path_against_refined_reference()

    assert comparison.topology == DIRECT_PLUS_BRANCHED_SPLIT_MERGE_REAR_PATH_TOPOLOGY
    ...
    assert comparison.metrics["direct_merged_delta_centered_mae"] < 0.10
    ...
    assert np.all(np.isfinite(comparison.observables["base_direct_merged_delta_db"]))


def test_direct_plus_branched_split_merge_rear_path_compare_writer_emits_csv_summary_and_observables(tmp_path: Path) -> None:
    outdir = tmp_path / "direct_plus_branched_split_merge_rear_path_compare"
    write_direct_plus_branched_split_merge_rear_path_compare_outputs(outdir)

    csv_path = outdir / "direct_plus_branched_split_merge_rear_path_compare.csv"
    observables_path = outdir / "direct_plus_branched_split_merge_rear_path_observables.csv"
    ...
    assert summary["case"] == "direct-plus-branched-split-merge-rear-path smoke"
    assert summary["topology"] == DIRECT_PLUS_BRANCHED_SPLIT_MERGE_REAR_PATH_TOPOLOGY
    assert summary["no_frontend_contract_change"] is True
    assert "direct_plus_branched_split_merge_rear_path_compare.csv" in summary["generated_files"]
    assert "direct_plus_branched_split_merge_rear_path_observables.csv" in summary["generated_files"]
