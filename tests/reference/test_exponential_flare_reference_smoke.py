...
from os_lem.reference_exponential_flare_smoke import (
    _relative_l2_change,
    _relative_scalar_change,
    build_exponential_model_with_segments,
    downsample_boundary_profile,
    evaluate_exponential_reference_smoke,
)


def test_exponential_reference_smoke_remains_finite_endpoint_consistent_and_refinement_bounded() -> None:
    report = evaluate_exponential_reference_smoke(
        build_exponential_model_with_segments,
    ...
    assert coarse_to_medium_flow <= 0.45
    assert medium_to_fine_flow <= 0.30
    assert medium_to_fine_flow <= coarse_to_medium_flow + 0.10
    assert medium_to_fine_pressure <= 0.20
    assert medium_to_fine_mouth <= 0.15
    assert medium_to_fine_endpoint_flow <= 0.15
