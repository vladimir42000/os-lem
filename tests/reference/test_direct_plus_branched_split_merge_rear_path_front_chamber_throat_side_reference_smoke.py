...
from os_lem.reference_direct_plus_branched_split_merge_rear_path_front_chamber_throat_side import (
    ...
    compare_direct_plus_branched_split_merge_rear_path_front_chamber_throat_side_against_refined_reference,
    write_direct_plus_branched_split_merge_rear_path_front_chamber_throat_side_compare_outputs,
)


def test_direct_plus_branched_split_merge_rear_path_front_chamber_throat_side_smoke_model_assembles_one_supported_skeleton() -> None:
...
def test_direct_plus_branched_split_merge_rear_path_front_chamber_throat_side_smoke_run_simulation_preserves_flow_balances() -> None:
    frequencies_hz = np.array([60.0, 120.0, 240.0, 480.0])
    result = run_simulation(build_direct_plus_branched_split_merge_rear_path_front_chamber_throat_side_model_dict(), frequencies_hz)
    ...
    np.testing.assert_allclose(
        ...
        result.series["direct_q_a"] + result.series["throat_up_q_a"],
        rtol=1e-10,
        atol=1e-12,
    )
    np.testing.assert_allclose(
        result.series["throat_down_q_a"],
        result.series["throat_up_q_b"] + result.series["front_cpl_q_b"],
        rtol=1e-10,
        atol=1e-12,
    )
    np.testing.assert_allclose(
        result.series["throat_down_q_b"],
...
def test_direct_plus_branched_split_merge_rear_path_front_chamber_throat_side_smoke_compare_produces_bounded_refinement_metrics() -> None:
    comparison = compare_direct_plus_branched_split_merge_rear_path_front_chamber_throat_side_against_refined_reference()

    assert comparison.topology == DIRECT_PLUS_BRANCHED_SPLIT_MERGE_REAR_PATH_FRONT_CHAMBER_THROAT_SIDE_TOPOLOGY
    ...
    assert comparison.metrics["zmag_shape_normalized_mae"] < 0.02
    assert comparison.metrics["front_spl_centered_mae"] < 0.12
    assert comparison.metrics["rear_sum_spl_centered_mae"] < 0.12
    assert comparison.metrics["total_spl_centered_mae"] < 0.18
    assert comparison.metrics["direct_merged_delta_centered_mae"] < 0.14
    assert comparison.metrics["junction_balance_mae_m3_s"] < 1.0e-10
    assert comparison.metrics["throat_side_balance_mae_m3_s"] < 1.0e-10
    assert comparison.metrics["split_balance_mae_m3_s"] < 1.0e-10
    assert comparison.metrics["merge_balance_mae_m3_s"] < 1.0e-10
    assert comparison.metrics["direct_merged_delta_span_db"] > 10.0
    assert comparison.metrics["rear_sum_minus_front_span_db"] > 20.0
    assert comparison.metrics["total_minus_front_span_db"] > 30.0
    assert comparison.metrics["front_to_throat_pressure_delta_span_pa"] > 5.0
    assert np.all(np.isfinite(comparison.base_total_spl_db))
    assert np.all(np.isfinite(comparison.refined_total_spl_db))
    assert np.all(np.isfinite(comparison.observables["base_front_to_throat_delta_pa"]))


def test_direct_plus_branched_split_merge_rear_path_front_chamber_throat_side_compare_writer_emits_csv_summary_and_observables(
    tmp_path: Path,
) -> None:
    outdir = tmp_path / "direct_plus_branched_split_merge_rear_path_front_chamber_throat_side_compare"
    write_direct_plus_branched_split_merge_rear_path_front_chamber_throat_side_compare_outputs(outdir)

    csv_path = outdir / "direct_plus_branched_split_merge_rear_path_front_chamber_throat_side_compare.csv"
    observables_path = outdir / "direct_plus_branched_split_merge_rear_path_front_chamber_throat_side_observables.csv"
    ...
    assert summary["case"] == "direct-plus-branched-split-merge-rear-path front-chamber throat-side smoke"
    assert summary["topology"] == DIRECT_PLUS_BRANCHED_SPLIT_MERGE_REAR_PATH_FRONT_CHAMBER_THROAT_SIDE_TOPOLOGY
    assert summary["no_frontend_contract_change"] is True
    assert "direct_plus_branched_split_merge_rear_path_front_chamber_throat_side_compare.csv" in summary["generated_files"]
    assert "direct_plus_branched_split_merge_rear_path_front_chamber_throat_side_observables.csv" in summary["generated_files"]
