...
from os_lem.reference_throat_side import (
    NO_FRONTEND_CONTRACT_CHANGE,
    THROAT_SIDE_TOPOLOGY,
    build_throat_side_model_dict,
    compare_throat_side_against_akabak,
    write_throat_side_compare_outputs,
)
...
def test_throat_side_reference_bundle_uses_existing_th_bundle() -> None:
...
def test_throat_side_smoke_compare_produces_bounded_current_metrics() -> None:
    comparison = compare_throat_side_against_akabak(REFERENCE_DIR)

    assert comparison.topology == THROAT_SIDE_TOPOLOGY
    ...
    assert comparison.metrics["zmag_overall_corr"] > 0.25
    assert comparison.metrics["zmag_overall_mae_ohm"] < 30.0
    assert comparison.metrics["pressure_db_overall_corr"] > 0.65
    assert comparison.metrics["pressure_db_overall_mae"] < 8.5
    assert comparison.metrics["pressure_phase_overall_mae_deg"] < 95.0
    ...
    assert np.all(np.isfinite(comparison.observables["blind_q_b_mag_m3_s"]))


def test_throat_side_compare_writer_emits_csv_summary_and_observables(tmp_path: Path) -> None:
    outdir = tmp_path / "throat_side_compare"
    write_throat_side_compare_outputs(REFERENCE_DIR, outdir)

    csv_path = outdir / "throat_side_compare.csv"
    observables_path = outdir / "throat_side_observables.csv"
    ...
    assert summary["case"] == "TH throat-side smoke"
    assert summary["topology"] == THROAT_SIDE_TOPOLOGY
    assert summary["no_frontend_contract_change"] is True
    assert "throat_side_compare.csv" in summary["generated_files"]
    assert "throat_side_observables.csv" in summary["generated_files"]


def test_throat_side_model_dict_matches_the_bounded_current_supported_case() -> None:
